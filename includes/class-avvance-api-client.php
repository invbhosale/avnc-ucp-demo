...
class Avvance_API_Client extends Avvance_API_Base {
	...
	public function create_financing_request( $order ) {
		...
		$payload = array(
			...
			'consumer'              => array(
				...
				'shippingAddress' => array(
					'street1'     => $order->get_shipping_address_1() ? $order->get_shipping_address_1() : $order->get_billing_address_1(),
					'street2'     => $order->get_shipping_address_2() ? $order->get_shipping_address_2() : $order->get_billing_address_2(),
		...
		$response = wp_remote_post(
			$this->base_url . '/poslp/services/avvance-loan/v1/create',
			array(
				'headers' => array(
	...
	public function void_transaction( $partner_session_id ) {
		...
		$response = wp_remote_post(
			$this->base_url . '/poslp/services/avvance-loan/v1/void',
			array(
				'headers' => array(
					'Authorization'  => 'Bearer ' . $token,
					'Content-Type'   => 'application/json',
	...
	public function refund_transaction( $partner_session_id, $amount ) {
		...
		$response = wp_remote_post(
			$this->base_url . '/poslp/services/avvance-loan/v1/refund',
			array(
				'headers' => array(
					'Authorization'  => 'Bearer ' . $token,
					'Content-Type'   => 'application/json',
